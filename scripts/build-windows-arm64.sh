...
#
# Unlike scripts/build-windows.sh (x86_64), Fedora doesn't package an
...
#      target either.
#   3. Cross-compiles a minimal static FFmpeg the same way build-windows.sh
#      does (SRR2_FFMPEG_STATIC), just re-targeted to aarch64.
...
#
# Requires docker or podman on the host. Output goes to build-windows-arm64/.
...
"$RUNTIME" run --rm \
	...
	sh -c '
		...
		TOOLCHAIN=/build/cmake/toolchain-arm64-mingw.cmake
		DEPS_ROOT=/build/build-windows-arm64-deps
		...
		echo "=== Building SRR2 ==="
		cd /build
		cmake -B build-windows-arm64 -DCMAKE_BUILD_TYPE=Release \
		...
		cmake --build build-windows-arm64 -j"$(nproc)"
		...
		# architectures copy.
		OUT_DIR=/build/build-windows-arm64/code
...
echo "Built: $REPO_ROOT/build-windows-arm64/code/SRR2.exe"
